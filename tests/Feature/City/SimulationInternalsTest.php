...
class SimulationInternalsTest extends TestCase
{
    ...
    public function test_farm_plus_mill_exact_amounts(): void
    {
        ...
        $city = $this->makeCity('mixexact', ['F02', 'P01'], ['粮食' => 50, '木材' => 123.5, '石料' => 77.25]);
        ...
        $this->assertEqualsWithDelta(140.0, $this->amountOf($city, '粮食'), 0.0001);
        // 面粉 = 8 × 0.5 × 10 = 40(资源行原本不存在,靠 upsert 插入)
        $this->assertEqualsWithDelta(40.0, $this->amountOf($city, '面粉'), 0.0001);
        // 无速率的资源一分不动
        $this->assertEqualsWithDelta(123.5, $this->amountOf($city, '木材'), 0.0001);
        $this->assertEqualsWithDelta(77.25, $this->amountOf($city, '石料'), 0.0001);
    ...
    public function test_capacity_and_maintenance_paths_exact(): void
    {
        ...
        $city = $this->makeCity('capexact', ['F02', 'D01', 'H01', 'S01'], ['粮食' => 200], 10);
        ...
        // 粮食速率 = +14(F02) − 1(D01 维护粮食) − 10×0.1(人口) = 12/min → 200 + 120 = 320
        $this->assertEqualsWithDelta(320.0, $this->amountOf($city, '粮食'), 0.0001);
        ...
        $this->assertEqualsWithDelta(14.0, $sim['grossProductionPerMin']['粮食'], 0.0001);
        $this->assertSame(['粮食'], array_keys($sim['grossProductionPerMin']), '容量类产出不得混进 grossProductionPerMin');
    ...
    public function test_gross_keys_reflect_recipe_rate(): void
    {
        $base = Carbon::parse('2026-01-01 00:00:00');
        Carbon::setTestNow($base);
        $city = $this->makeCity('grosshalf', ['F02', 'P01'], ['粮食' => 50]);
        ...
        $this->assertEqualsWithDelta(4.0, $sim['grossProductionPerMin']['面粉'], 0.0001, 'gross 面粉产出 = 8 × 0.5');
        $this->assertEqualsWithDelta(5.0, $sim['grossConsumptionPerMin']['粮食'], 0.0001, 'gross 粮食消耗 = 10 × 0.5');
        $this->assertEqualsWithDelta(14.0, $sim['grossProductionPerMin']['粮食'], 0.0001, 'F02 的 gross 粮食产出不打折');
        $this->assertSame(['粮食'], array_keys($sim['grossConsumptionPerMin']), '只有配方投入进 gross 消耗');
        // 净速率 = gross 产出 − gross 消耗(此城人口 0、无维护粮食)
        $this->assertEqualsWithDelta(4.0, $sim['ratesPerMin']['面粉'], 0.0001);
        $this->assertEqualsWithDelta(9.0, $sim['ratesPerMin']['粮食'], 0.0001, '14 − 5');
    }
    ...
    public function test_gross_keys_sum_per_instance(): void
    {
        $base = Carbon::parse('2026-01-01 00:00:00');
        Carbon::setTestNow($base);
        $city = $this->makeCity('grossfull', ['P01', 'P01'], ['粮食' => 1000]);

        Carbon::setTestNow($base->copy()->addMinutes(10));
        $sim = SimulationService::simulate($city->fresh());

        $this->assertEqualsWithDelta(16.0, $sim['grossProductionPerMin']['面粉'], 0.0001, '两座磨坊 gross 产出 8×2');
        $this->assertEqualsWithDelta(20.0, $sim['grossConsumptionPerMin']['粮食'], 0.0001, '两座磨坊 gross 消耗 10×2');
        $this->assertEqualsWithDelta(160.0, $this->amountOf($city, '面粉'), 0.0001);
        $this->assertEqualsWithDelta(800.0, $this->amountOf($city, '粮食'), 0.0001);
    }
    ...
    public function test_gross_keys_present_when_elapsed_zero(): void
    {
        $base = Carbon::parse('2026-01-01 00:00:00');
        Carbon::setTestNow($base);
        $city = $this->makeCity('grosszero', ['P01'], ['粮食' => 1000]);

        $sim = SimulationService::simulate($city->fresh());

        $this->assertSame(0, $sim['elapsedSeconds']);
        // 需求为 0 → 满足率取 1 → 返回的是无约束名义速率
        $this->assertEqualsWithDelta(8.0, $sim['grossProductionPerMin']['面粉'], 0.0001);
        $this->assertEqualsWithDelta(10.0, $sim['grossConsumptionPerMin']['粮食'], 0.0001);
        $this->assertEqualsWithDelta(1000.0, $this->amountOf($city, '粮食'), 0.0001, 'elapsed=0 不落库');
    }
    ...
    public function test_batch_upsert_writes_every_resource_exactly(): void
    {
        ...
        $city = $this->makeCity('upsertmix', ['F02', 'P01', 'E02'], ['粮食' => 60, '木材' => 100, '石料' => 50]);

        Carbon::setTestNow($base->copy()->addMinutes(10));
        SimulationService::simulate($city->fresh());

        // 粮食需求 100、库存 60 → P01 recipeRate 0.6;木材需求 60、库存 100 → E02 recipeRate 1.0
        $this->assertEqualsWithDelta(140.0, $this->amountOf($city, '粮食'), 0.0001, '60 + (14 − 10×0.6)×10');
        $this->assertEqualsWithDelta(48.0, $this->amountOf($city, '面粉'), 0.0001, '8 × 0.6 × 10');
        $this->assertEqualsWithDelta(40.0, $this->amountOf($city, '木材'), 0.0001, '100 − 6×10');
        $this->assertEqualsWithDelta(50.0, $this->amountOf($city, '燃料'), 0.0001, '5 × 10');
        $this->assertEqualsWithDelta(50.0, $this->amountOf($city, '石料'), 0.0001, '无速率的资源不被 upsert 波及');
    ...
    public function test_repeated_upsert_does_not_duplicate_rows(): void
    {
        $base = Carbon::parse('2026-01-01 00:00:00');
        Carbon::setTestNow($base);
        $city = $this->makeCity('upsertloop', ['F02', 'P01'], ['粮食' => 500]);
        ...
        $this->assertEqualsWithDelta(240.0, $this->amountOf($city, '面粉'), 0.0001);
        $this->assertEqualsWithDelta(620.0, $this->amountOf($city, '粮食'), 0.0001);
    }
