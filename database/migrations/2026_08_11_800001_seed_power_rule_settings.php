...
// 补写 M.1 电力系统的规则参数行(game_settings)。
//
...
// 运营也看不出「这一项到底有没有被人动过」。与 700002(事件)/ 600003(工具)同一做法。
//
// 幂等:逐 key 先查后插,已存在的行完全不动 —— 被运营改过值的库绝不能被迁移覆盖回默认值。
return new class extends Migration
{
    // 本支迁移负责的 key:GameSetting 里所有 power_ 前缀的登记项
    private function keys(): array
    {
        return array_values(array_filter(
            array_keys(GameSetting::DEFINITIONS),
            fn ($key) => str_starts_with($key, 'power_')
        ));
    }
