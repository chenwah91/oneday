...
// 新增 M3-D2 工具的 6 条后台可调设定(2 条开关 + 4 条数值)。
//
// 为什么要单独一支迁移(与 500002 市场那支逐字同理):2026_08_10_500001 建表时按当时的
// GameSetting::DEFINITIONS 灌行,已经跑过那支迁移的库(开发 apg / 线上)不会自动补上后来新增的 key,
// 后台设置页就看不到工具参数,运营出事时无从调起(GameSetting::get 仍有默认值兜底,不影响玩法)。
//
...
return new class extends Migration
{
    ...
    // 将来别的系统再加 item_* 开头的 key,不该被这支历史迁移追认)
    private const KEYS = [
    ...
    public function up(): void
    {
        foreach (self::KEYS as $key) {
            ...
            DB::table('game_settings')->insert([
                ...
                // 读回来就变成 int —— 数值型设定的类型会在「落库再读出」的往返里悄悄漂移
