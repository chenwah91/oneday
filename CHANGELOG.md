...
## [v1.10.0] — W15 后台独立会话与管理员分离(2026-08-16)

用户需求:「后台只允许管理员,这个需要和用户分开」+「玩家登入和 admin 登入需要完全分开」。**0 支新迁移**,测试 1062 → **1091 passed**。

起因是用户实测踩坑:游戏与后台同源共用一个浏览器 session,两边都打 `/api/auth/login` 写同一个 web guard 登录键 —— 管理员登进后台后再在游戏页登录玩家号,后台每个请求就变成玩家身份,整片 `/api/admin/*` 当场 403(审计实证:12:26:32 chenwah_admin 登录 → 12:26:56 chenwah91 登录 → 之后全部 `NOT_ADMIN role=player`)。

### 两套会话彻底分家

- 新增 **`admin` guard**(同 `users` provider,但 session 登录键按 guard 名分开:`login_web_*` / `login_admin_*`)。同一个浏览器可以同时挂着「玩家已登录」和「管理员已登录」,互不覆盖 —— **不需要额外 cookie**。
- 新增后台专用端点 `POST /api/admin/auth/{login,logout}`;`api/admin` 组的 `auth:web` → **`auth:admin`**。两道闸各管一件事:`auth:admin` 管「有没有从后台的门进来」,`admin`(EnsureAdmin)管「现在的角色够不够」(登录后被降级靠它兜住)。
- **后台只接受管理员**:玩家账号即使用户名密码全对也进不去。校验顺序 `密码 → 封禁 → staff 角色`,后两道一律排在密码之后 —— 排在之前等于白送一个「这个号被封了 / 这个号是管理员」的枚举面。密码错与账号不存在返回**逐字节相同**的 401。
- `LoginController` / `RegisterController` 显式写 `guard('web')`,不再赌默认 guard。

### 审计:玩家与后台完全分开(用户 2026-08-16 追加)

后台三个认证动作各有各的 action code,不再复用 `AUTH.*` 靠 `surface` / `actor_type` 字段区分——运营按 action 一查就混在一起,「有人在敲后台门」和「有人在爆破玩家号」这两件处置优先级完全不同的事被算成同一堆。

- 新增 **`ADMIN.LOGIN_FAILED`** / **`ADMIN.LOGOUT`**(与既有 `ADMIN.LOGIN` 成套)。`WHERE action LIKE 'ADMIN.%'` 就是后台认证的全部活动。
- **唯一刻意的例外是限流计数器**:两道门共用同一个按账号的 key。分开反而削弱安全 —— 各开一个 5 次窗口 = 每个账号白送 10 次猜测。

### 对抗性审查修掉的 6 个问题

实现完成后跑了一轮 53 个 agent 的多视角对抗审查(每条发现由两个立场不同的反驳者验证),11 条经反驳仍成立,去重后 6 个:

- **[high] 登出留下可重放的孤儿会话**(本波自伤,审查抓出)。为保护管理员会话把 `invalidate()` 换成了 `regenerate()` —— 而 `regenerate()` = `migrate(false)` **不销毁旧记录**:`logout()` 摘登录键只发生在内存属性里,请求末尾被写到**新** id,旧 id 那行原封不动留着两个身份。登出成了「会话分叉」而非「会话吊销」(CWE-613 / ASVS 3.3.1)。正解 **`regenerate(true)`**:销毁旧行 + 换 id + 不 flush,吊销与不掀桌子两个目的同时满足。
- **[high] `EnsureNotBanned` 连坐**:它仍用 `invalidate()` 清空整个 session,被封玩家的一次游戏请求会把同浏览器的管理员一起踢下线(甚至就是管理员自己点的封禁把自己踢了)。改为 `Auth::logout()`(走默认 guard,即判定所依据的那个身份)+ `regenerate(true)`。
- **[medium] 后台登录被别人的封禁误伤**:登录口没挂 `auth:*`,默认 guard 仍是 web,于是按同浏览器那个**玩家**的封禁状态判 —— 管理员看到「账号已封禁」而他根本没被封。改为豁免 `api/admin/auth/login`。
- **[high] 玩家会话探测后台 API 完全无痕**:改用 `auth:admin` 后,`Authenticate` 在 `EnsureAdmin` 之前抛异常,原本必然写下的 `SECURITY.AUTHORIZATION_FAILED` 整条消失 —— 「已登录玩家在扫后台」这个最有价值的入侵信号没了(§60/§67)。在异常处理里补回,`reason_code=NO_ADMIN_SESSION` 与 `NOT_ADMIN` 刻意分开;**只在持有玩家会话时记**,未登录访客的 401 是常态噪音,全记会淹掉真信号。
- **[medium] 登录失败日志两头栽赃**:审计行 `user_id` 恒 NULL(运营按账号查不出谁在爆破某管理员),而 `SecurityLogger` 的 `??` 把显式的 `null` 退回成 `$request->user()` —— 后台登录口默认 guard 是 web,退回来的正是同浏览器那个**无关玩家**。改为 `array_key_exists` 语义,并把 `user_id` 从 filter 结果里剔除(归属只有一个来源)。
- **[medium] 后台登录后可能进全白页**:`loadMe` 非 403 失败被静默吞掉,注释说「仅角色徽标留空」,实际权限清单也空了 → 14 个面板全部隐藏 → 顶栏正常、正文全白、零提示,刷新又撞一次退回登录,陷入死循环。改为退回登录视图并说明原因。

### 需要知道的

- 玩家访问后台 API 的语义由 **403 变 401**(`auth:admin` 先拦);403 那条路径仍可达(持后台会话但角色被降级),两条各有测试。
- 「登出销毁旧 session 记录」**在 HTTP 层测不出来**(Laravel 测试客户端不回传 session cookie,每请求新 id),故用源码断言 + Store 层三写法对照实验两条合钉。
- 冒烟账号已按授权清理(`scripts/cleanup_smoke_accounts_w15.php`,dry-run + 事务 + 白名单断言):5 个账号 / 5 座城市 / 相关运行时行,**审计 351 行原样保留**(append-only)。
- 本项目测试**不能并发跑**(共用一个测试库,`RefreshDatabase` 会互相踩踏)——这也是此前那个"偶发失败"的真正原因。

## [v1.9.0] — W16 建造面板按规则流程开放(2026-08-15)

用户需求:「跟着游戏的规则流程,每个阶段只开放可以用的建筑;可以用的建筑如果已经建好或者已经建到 max 了就变成半透明」。**0 支新迁移**,测试 1061 → **1062 passed**。

### 建造面板改三态(`public/game/js/ui/build-panel.js`)

原先 94 座建筑一次性平铺在一条横向滚动条里,只按时代置灰。现在按服务端 `BuildService` 的闸门顺序(时代 → 科技 → 数量上限)分三态:

- **可建** — 正常可点,`order:0` 排最前。
- **已满** — 已达 `max_count`,**半透明**(0.42)不可点,`order:1` 排其后,状态行写「已达上限 1/1」;未满但已建过的显示「已有 1/18」,仍可点。
- **未解锁** — 时代或科技不够,**默认整条隐藏**,由表头「🔒 未解锁 N」按钮切换出来看目标,状态行写明差什么(「需要时代 定居时代」/「需要科技 部落治理」)。

刷新只改 class / `flex order` / `hidden`,不重建 DOM——每次快照重建 94 个按钮既浪费,也会抹掉放置中的 active 高亮。

### 定义端点补齐闸门字段(`GET /api/definitions/buildings`)

新增 `tech_id` / `tech_name` / `era_name` 三个字段(`leftJoin technology_definition`,无前置科技的建筑不会被内连接吃掉)。此前只下发时代,前端无从判断科技闸门,玩家会看到一堆点下去必然 `TECH_NOT_UNLOCKED` 的按钮;科技与时代的中文名一并给出,免得为一句提示再拉 50 节点的科技定义。

### 需要知道的行为变化

- 现行数值里 **94 座建筑全部带前置科技**,而新城市 0 科技——所以**新手首次打开「建造」是空列表**,面板给出可操作提示:「先去『科技』研究前置科技,或点右上『未解锁』看各建筑差什么」。测试 `DefinitionApiTest` 钉住了这个前提,哪天有建筑不再需要科技会红,提醒复核该文案。
- 施工中的实例**计入**数量上限(与 `BuildService` 的 `count()` 同口径),否则施工期间会露出一个点下去必然 `BUILDING_LIMIT_REACHED` 的按钮。
- 这里只是显示层。玩家改 JS 点亮按钮照样过不了服务端(§66),口径不同也只是提示不准。
- PWA 缓存 `apg-v12` → **`apg-v13`**。

## [v1.8.0] — W14 后台定义 CRUD 波(2026-08-13)

用户五点需求的落地:四张定义表**可在后台手动新增**、科技按分支分类、NPC/市场定义可改可增、玩家详情资源行一键跳补偿。**0 支新迁移**(四张表字段本就齐备),测试 1045 → **1061 passed**。

### 四个「新增」端点(全部走既有 11 步管线:allowlist → 强制 reason 2~80 字 → 事务 + 行锁 → 审计 `ADMIN.CONFIG_CHANGE`(create 语义)→ GDV bump → 缓存 flush)

- **建筑等级加一行**:`level` 由服务端算(当前最高级 + 1,锁内重算),客户端不传——等级连续性从构造上不可破;后台表单**预填最高级数值副本**(含产出/投入/建造成本三个 JSON),运营在此基础上改。
- **新增科技**:tech_id 唯一 + 格式校验,`era_key`/`branch`/前置科技/解锁建筑逐项外键与枚举校验(前置不可引用自己)。
- **新增 NPC**:全列表单,`npc_id` 格式 `N\d{3}`,`name_key` 服务端派生;`trait_json` **新增开放**(按 Seeder 同一口径逐条构造 `ModifierSpec` 校验 + 数值护栏,只保留已知键),编辑仍锁死结构(强度走 `trait_multiplier`)。
- **新增市场定义**:仅对尚未上市的资源开放,`rs_code`/`first_era` 服务端派生,`min ≤ base ≤ max` 跨字段校验。裁决记录:`market_category`(11 个市场分组)与 `resource_definition.category`(6 个资源类)**语义不同、派生不出来**,故由前端在库内已有分组里选、服务端 Fail Closed 校验,新分组走迁移。

### 编辑器扩全(修旧挂账)

- **NPC**:可编辑列 6 → 13(中文名/类别/时代/稀有度/来源/招募文案/特性描述);**旧挂账补齐**——6 个数值列逐列有上限校验(此前只有 `trait_multiplier` 有),新增「改单列时与另一列现值合并校验」拦 `initial_skill_level > max_level`。
- **市场**:补 `note`(此前无任何入口能写);新增 `min ≤ base ≤ max` 三元组校验——原先只拦 `min>max`,基础价掉出区间没人管,会让价格永远贴边抖、波动率/弹性两个旋钮「改了没反应」。
- **通用渲染器支持文本列**:后端把中文名/描述/备注开放成可编辑后,这些列若仍按数值渲染就是个填不进字的数字框;`fieldMeta.text` 起渲染成文本框(可留空,`required` 时才拦),枚举列渲染成下拉。

### 后台界面

- **科技面板按分支分组**:5 组标题带条数,组内**按时代正序**(后端按 tech_id 字符串排会排成 I/II/III/IV/**IX**/V…,运营顺着时代读表才顺手);行是移动不是重建,未提交的输入不丢。
- **玩家详情 → 补偿联动**:资源现况每行一颗「补偿 / 扣减」按钮 → 跳补偿面板,`city_id` 预填 + 自动查询 + **该资源自动选中**;hash 参数用完即摘,刷新/切 tab 不会重复触发。

## [v1.7.0] — W13 后台强化波(2026-08-13)

用户两点后台需求的落地(双线并行):玩家详情全景页 + 建筑等级无上限与 Excel 批量管理。**0 支新迁移**(表结构不变),测试 1018 → **1045 passed**。

### 玩家详情全景页(后台)

- `GET /admin/api/players/{id}` 从摘要扩展为**全景端点**:账号 / 城市全字段 / 资源现况 / 建筑(含名称·等级·状态·坐标·工人) / NPC(含派驻楼) / 科技 / 工具(含耐久·装备楼) / 事件(生效中 + 最近 10 条已结算) / 最近 20 笔市场交易(带 delta) / 最近 20 条审计。每分区一条有界 SQL(零 N+1),读路径**不跑结算**(数值为最近结算时点快照,页首标注),敏感列绝不出库,旧键全部保留。
- 玩家面板列表/详情双视图:行点击 → `#players?player=N` 全景页;**内嵌三个操作区**(按权限显隐):资源补偿/扣减(强制 reason+幂等键)、按城触发事件、封禁/解封;「查看完整审计」带过滤跳审计面板。
- 口径说明:系统暂无跨玩家关联(联盟/好友),「有关系的数据」= 玩家自身全量数据,页面已向运营写明。

### 建筑等级无上限 + Excel 批量导入导出

- **等级上限数据驱动**(§13):删除 UpgradeService 写死的「3 级封顶」——定义表里有 `(building_id, level+1)` 行就能升,没有即满级(`BUILDING_LIMIT_REACHED` 语义不变)。玩家端定义接口每栋建筑新增 `max_level`,建筑详情的 `Lv x / MAX` 与升级按钮跟随(老响应回落 3);编辑器 level 校验 1~3 → 1~255(tinyint 物理上限)。全库 3 级假设逐处审计:档位判断(NPC 槽位 ≥3 用 L3 档)确认对无上限天然成立,保持不动。
- **Excel 导出/导入**(零新依赖,PHP 内建 ZipArchive+SimpleXML 自写极简 xlsx 读写器,含往返单测):
  - 导出:`GET …/building-levels/export` 下载全部等级行(表头即导入 allowlist,JSON 列原文进单元格)。
  - 导入:`POST …/building-levels/import`(file + 强制 reason)。批量版 11 步管线:表头 allowlist → 逐行校验(building_id 必须已存在 / level ≥1 整数 / 数值列 FIELD_MAX / JSON 合法且过资源 allowlist)→ **等级 1..N 连续性不变量**(断档 422)→ 全量通过才开事务(行锁,diff 只写变化行+补新行,**绝不 DELETE**)→ 一次 GDV bump + 一条汇总审计;错误按 `{row, column, reason}` 逐行返回(上限 50 条)。
  - 后台建筑等级面板配「导出 Excel / 导入 Excel」工具栏,成功显摘要、失败逐行列错。
- 升级路径的缺料明细(`details.missing[]`,与 v1.6.0 建造同款)随本版 UpgradeService 一并落地,建筑详情升级失败可显示「资源不足:木材还差X」。
