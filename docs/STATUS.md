...
**M2 进行中(多 agent 波次开发,用户已授权持续推进至叫停)。** 已完成:v1.0.1 修复 5 个 M1 缺陷(88 测试)→ 波次1(结算内核 per-instance 化+七乘数占位/安全可观测性/建筑详情面板,102 测试)→ 波次2(资源 ID 全英文化+粮耗 0.03=V3.1.2、E10 GDV 贯通、E6 五级管理员角色,137 passed)→ 波次3(定义表枚举值英文化=**V3.1.3**、M2-C1 人口/劳动力:分段结算+工人分配 API+人口增长/迁出/饥荒,存档人口 10→30,**172 passed**)。用户三点拍板(2026-08-10):①没派工人就不生产是预期玩法,不自动派工,前端派工 UI 要立即补;②API 字段一律小写(存量 camelCase 全链改 snake_case);③「只减不增」放行宽松执行,开关进后台 game_settings。波次4 已完成(**202 passed**):①游戏 API 契约全 snake_case(请求+响应,含前端全链与测试)+前端派工 UI(建筑面板派工控件+HUD 劳动力,浏览器实测含 409 恢复与 400px 触控)+SW apg-v5;②E7 管理员补偿 ADMIN.COMPENSATION(全安全链+后台面板)+game_settings 后台开关(worker_assign_allow_decrease_always / worker_gate_enabled,接入 WorkerService 与结算内核)+确认 worker_required 后台可编辑。波次5 已完成(**242 passed**):M2-C2 幸福/健康/治安(happiness 列+目标合成+快落慢升-1.0/分+缺粮5分钟起扣+happinessFactor 接入增长,health/security 派生覆盖率,HUD 😊❤️🛡️)、B1 科技研究(POST /api/city/research 全安全链+懒完成解锁+科技面板 50 节点浏览器实测;时代判定暂由已解锁科技派生,待 B6 era_key 替换;tech 乘数未接线=B3,建造科技闸门=B4)。波次6 已完成(**281 passed**):C3 财政/治理(税收 0.02×1.5^时代×治理四档效率进分段结算,维护欠费半停工×0.5 取代白嫖,治理容量单一来源=output_json)、B6 时代升级(cities.era_key/era_order 列,POST /api/city/era/upgrade 纯门槛逐维校验+快照 era 块+前端时代区块/建造置灰,建造与研究接时代闸门,研究收紧为只开放当前时代)。注意:存档城全部回填时代 I,再建 F02(时代II)需先升时代或后台调 era_order。波次7 已完成(**329 passed**):C5 建筑生命周期(施工计时 constructing/upgrading 三态+懒完工窗口口径宁可少产+升级取消退70%+拆除返还50%/70%三态+B4 建造科技闸门,升级期间停产=§3.2 明文)、C4 物流乘数(§10.7+§3.3 合成曲线,transport_capacity 真实参与,时代 I 不计需求)+§13 硬上限 2.75×/3.25×+财政预警黄红两档进快照与 HUD。波次7 遗留:住宅升级保留50%人口容量(§3.2 后半句,内核)、constructing 建筑可派工占池待裁决、物流「升代产量断崖」可调参待用户定。波次8 已完成(**376 passed**),M2 主体功能全部落地:B3 科技乘数接线(v3.2 §5 effect_code=同分支+2%,建筑按自身 tech_id 反查分支纯数据驱动,满分支 1.20×)+住宅升级保留50%人口容量;C6 安全回归(23 类攻击场景全实测,修复 1 个高危:超长 X-Request-ID 压制审计的 1406 漏洞,新增 31 条攻击测试)。C6 裁决:研究不受拆楼影响维持现状;未完工建筑派工占池维持现状(预派=预留)。进行中:M2 收官(CHANGELOG v1.1.0+deploy.md+浏览器全链 E2E+小加固)、M3 规划(D 轨道 NPC/工具/市场/事件 backlog)。待用户拍板(不阻塞):两份映射草案审批、开发库脏数据清理(city4 容量行+实测账号 uitest0810a/techsmoke)、v3.2.md 文档同步(字段 snake_case+科技 branch 名)、**定义表三列双口径(governance_bonus/defense_score/happiness_bonus 与 output_json 不一致共 150+行,现按 output_json 单一来源,列被忽略且后台可编辑无效,删列还是改口径待裁决)**。
