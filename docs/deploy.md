...
⚠️ **上传前先备份(数据库 + 现有代码)**——本仓库的完整部署涉及数据库结构变更(迁移)与定义数据(seed),上传前必须先在 cPanel/phpMyAdmin 对生产数据库做一次完整导出备份,并保留一份现有线上代码的副本,确认可回退后再继续。**v1.2.0 → v1.7.0 的增量升级会跑 9 支新迁移,其中 `2026_08_13_200002` 改 `users` 表结构(加封禁两列,存量行保持 NULL)、`2026_08_13_300001` 删 `building_definition` 五个死列——两处结构变更,备份纪律不打折**(见下方「路径 B」)。v1.6.0 / v1.7.0 / v1.8.0 三版是**纯代码波,0 支新迁移**(玩家界面重组 + 后台强化 + 后台定义 CRUD);另注意上线后**后台 Excel 导入建筑等级、以及四张定义表的「新增」功能**都会改定义数据——运营操作前同样先备份。
...
>
> 版本对应关系:代码 `v1.8.0` · 数值 `game_data_version = V3.8.1`(生产库基线;开发/运营期后台编辑、Excel 导入与定义新增会继续 bump 补丁位) · PWA 缓存 `apg-v12` · 迁移文件 **76 支** · 后台规则参数 **154 项** · 测试 **1061 passed**。
...
v1.8.0 是 **R1 第一基础版本**(v1.5.2 收口后追加三个纯代码波:v1.6.0 玩家界面重组、v1.7.0 后台强化、v1.8.0 后台定义 CRUD,均无新迁移)。同一份代码有两种上线场景,步骤差别很大 —— **先对号入座,再往下读**:

| | **路径 A:首次上线(全新库)** | **路径 B:v1.2.0 → v1.8.0 增量** |
|---|---|---|
...
| 二、5.1 SW 版本 | ✅ 确认 `apg-v12` | ✅ 确认 `apg-v12`(v1.2.0 是 `apg-v10`,**这一版必须变**,否则老玩家拿不到新前端) |
| 二、6 建管理员 | ✅ | ❌ 已有 |
