...
>
> 版本对应关系:代码 `v1.7.0` · 数值 `game_data_version = V3.8.1`(生产库基线;开发/运营期后台编辑与 Excel 导入会继续 bump 补丁位) · PWA 缓存 `apg-v12` · 迁移文件 **76 支** · 后台规则参数 **154 项** · 测试 **1045 passed**。
...
v1.7.0 是 **R1 第一基础版本**(v1.5.2 收口后追加两个纯代码波:v1.6.0 玩家界面重组、v1.7.0 后台强化,均无新迁移)。同一份代码有两种上线场景,步骤差别很大 —— **先对号入座,再往下读**:

| | **路径 A:首次上线(全新库)** | **路径 B:v1.2.0 → v1.7.0 增量** |
|---|---|---|
